...
class ABAE(nn.Module):
    ...
    def forward(self, inputs, neg_samples, eps=1e-06):
        ...
        neg_lengths = neg_samples[:, :, 0].view(self.opt.batch_size * self.opt.neg_size)
        neg_inputs = neg_samples[:, :, 2:]
        neg_inputs = neg_inputs.view(self.opt.batch_size * self.opt.neg_size, 300)
        ...
        neg_inputs = neg_global_content_embed.view(self.opt.batch_size, self.opt.neg_size, 300)
