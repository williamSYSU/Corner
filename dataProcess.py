...
def weakly_data_process(opt):
    ...
    if opt.need_pos is True:
        input_sentence_1 = 108947 + np.zeros((len(pairs_pos), 604))
        input_sentence_1[:, 302:] = 300 + np.zeros((len(pairs_pos), 302))
        input_sentence_1 = input_sentence_1.astype(np.int)
        input_sentence_2 = 108947 + np.zeros((len(pairs_neg), 604))
        input_sentence_2[:, 302:] = 300 + np.zeros((len(pairs_neg), 302))
        input_sentence_2 = input_sentence_2.astype(np.int)
    ...
    def cal_sentence_index():
        for line in range(len(pairs_pos)):
            ...
            if opt.need_pos is True:
                input_sentence_1[line][302:length + 302] = [x for x in range(length)]
        ...
        for line in range(len(pairs_neg)):
            ...
            if opt.need_pos is True:
                input_sentence_2[line][302:length + 302] = [x for x in range(length)]
    ...
    input_pos_train = input_sentence_1[:int(len(input_sentence_1) * opt.weakly_sr), :]
    input_neg_train = input_sentence_2[:int(len(input_sentence_2) * opt.weakly_sr), :]
    # print(input_sentence_1[0])

    input_pos_test = input_sentence_1[int(len(input_sentence_1) * opt.weakly_sr):, :]
    input_neg_test = input_sentence_2[int(len(input_sentence_2) * opt.weakly_sr):, :]
    ...
    return all_data, final_embedding, np.array(input_pos_test[0:opt.weakly_test_samples, :]), np.array(
...
def classification_process(opt):
    ...
    input_sentence = 108947 + np.zeros((len(pairs_classifier), 204))
    input_sentence = input_sentence.astype(np.int)

    if opt.need_pos is True:
        input_sentence = 108947 + np.zeros((len(pairs_classifier), 408))
    ...
    pos_sentence_train = pos_sentence[:int(len(pos_sentence) * opt.split_rate), :]
    pos_sentence_valid = pos_sentence[int(len(pos_sentence) * opt.split_rate): int(
        len(pos_sentence) * (opt.split_rate + 1) / 2), :]
    pos_sentence_test = pos_sentence[int(len(pos_sentence) * (opt.split_rate + 1) / 2):, :]

    neg_sentence_train = neg_sentence[:int(len(neg_sentence) * opt.split_rate), :]
    neg_sentence_valid = neg_sentence[int(len(neg_sentence) * opt.split_rate): int(
        len(neg_sentence) * (opt.split_rate + 1) / 2), :]
    neg_sentence_test = neg_sentence[int(len(neg_sentence) * (opt.split_rate + 1) / 2):, :]
...
def aspect_extract_data_process(opt):
    ...
    input_sentence = 108947 + np.zeros((len(pairs_all), 302))
    input_sentence = input_sentence.astype(np.int)

    if opt.need_pos is True:
        input_sentence = 108947 + np.zeros((len(pairs_all), 604))
        input_sentence[:, 302:] = 300 + np.zeros((len(pairs_all), 302))
        input_sentence = input_sentence.astype(np.int)
    ...
    def cal_sentence_index():
        for line in range(len(pairs_all)):
            ...
            # if input_sentence[line][2] == 108947:
            #     print("there is a 0")
            #     print(input_sentence[line][0])

            if opt.need_pos is True:
                input_sentence[line][302:length + 302] = [x for x in range(length)]
...
#     weakly_data_process(opt)
#     classification_process(opt)
#     print("Hello world")
